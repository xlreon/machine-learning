...
def kcluster(rows,distance=pearson,k=4):
    ...
    # Determine the mimimum and max values for each point
    ...
    for t in range(100):
        ...
        # Moving the centroids to the average of their members
            for i in range(k):
                avgs=[0.0]*len(rows[0])
...
def tanamoto(v1,v2):
    ...
    for i in range(len(v1)):
        ...
        if v1[i]!=0 and v2[i]!=0: shr+=1 # present in both (intersection)
    ...
    for m in range(0,1000):
        ...
        for k in range(n):
            for j in range(n):
                ...
                grad[k][1]+=((loc[k][0]-loc[j][1])/fakedist[j][k])*errorterm

                # Kepp track of the total error
                totalerror+=abs(errorterm)
            print (totalerror)

            # Ift he answer got worse by moving the points, we are done
